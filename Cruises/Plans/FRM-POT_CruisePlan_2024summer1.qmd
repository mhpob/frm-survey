...
subtitle: "July 15-17, 2024   *F/V Integrity*   West Ocean City, MD"
format: html
editor: visual
echo: false
message: false
...
stations <- rbind(ocmd, a5c, ctrl_sites, exp_sites)
...
rec_seq <- data.table(start =stations$station[path[1:(length(path) - 1)]],
                         ...
                         sttime = c(as.POSIXct(params$start$d2) + 30*60, rep(NA, 7)),
                         endtime = c(as.POSIXct(params$start$d2), rep(NA, 7)),
                         pots = c(as.POSIXct(params$start$d2), rep(NA, 7)))
...
🔲 Sonardyne Release transponder/box
:::
:::
...
### Misc
- A-5C: 38 20.160, -74 43.392
  - Payload: 38 20.160, -74 43.356; 100#: 38 20.166, -74 43.422
  - Release Serial: 303531-003
  - Release Addr: 111.1
...
leaflet() |> 
  ...
  addCircleMarkers(
    lat = c(38.3360, 38.3361,
            38.3319, 38.3311),
    lng = c(-74.7226, -74.7237,
            -74.9491, -74.9489),
    fillColor = 'black',
    color = 'black'
  ) |> 
  addCircleMarkers(
    lat = c(38.3360, 38.3315),
    lng = c(-74.7232, -74.9491),
    fillColor = 'red',
    color = 'red',
  ) |> 
...
-   `r format(deploy_seq$endtime[7], '%H:%M')` Arrive **`r deploy_seq$end[7]`**. Attempt recovery.
:::
:::
