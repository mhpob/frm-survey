---
title: 'Pot Survey -- June 2023 (2)'
subtitle: "June 26 - 28, 2023   *F/V Integrity*   West Ocean City, MD"
format: html
editor: visual
echo: false
message: false
...
stations <- rbind(ocmd, rtwb, ctrl_sites, exp_sites)
...
deploy_seq <- data.table(start = stations$station[path[1:(length(path) - 1)]],
                         ...
                         sttime = c(as.POSIXct(params$start$d1) + 30*60, rep(NA, 7)),
                         endtime = c(as.POSIXct(params$start$d1), rep(NA, 7)),
                         pots = c(as.POSIXct(params$start$d1), rep(NA, 7)))
...
no_rtwb_station <- stations[stations$station != 'RTWB',]
no_rtwb_path <- path[path != 2]
no_rtwb_path[no_rtwb_path >1] <- no_rtwb_path[no_rtwb_path >1]- 1

rec_seq <- data.table(start = no_rtwb_station$station[no_rtwb_path[1:(length(no_rtwb_path) - 1)]],
                         end = no_rtwb_station$station[no_rtwb_path[2:length(no_rtwb_path)]],
...
for(i in 2:(length(no_rtwb_path)-1)){
...
-   Scientific crew (DS, KS, EK)
    -   08:00 - 09:00 DS/KS/EK pack CBL truck (2017 Silverado) at Solomons, MD. Drive to MD Coastal Bays Program office.
...
-   Full crew (DS, KS, EK, BH, mate)
    -   13:30 - 14:00 Unload pots. BH/mate begin loading pots onto vessel.
    -   14:00 - 16:00 DS/KS/EK drive to MD Coastal Bays Program. Repeat pot loading/return/unloading.
    -   16:00 - 16:30 Drop trailer off at MDCBP. Return to W Ocean City.
    -   16:30 Check into Microtel
...
-   Scientific crew
    -   `r format(as.POSIXct(params$start$d2) - 20*60, '%H:%M')` Check out of Microtel
-   Full crew
...
-   `r format(deploy_seq$endtime[7], '%H:%M')` Arrive **`r deploy_seq$end[7]`**. Deploy pots.
-   `r format(deploy_seq$pots[7], '%H:%M')` Finish deploying pots. Depart for **`r deploy_seq$end[8]`**
