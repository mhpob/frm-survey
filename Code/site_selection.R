...
# Consideration: near early stages (far?)

library(sf); library(ggplot2); library(concaveman); library(TSP)
...
exp_sites <- towers |> 
  DT(grepl('^A-.*-[456789]', station) & location == 'inner') |>
  DT(sample(.N, 2)) |> 
  DT(, location := NULL) |> 
  st_as_sf()
...
geom_sf(data = towers, aes(geometry = geometry)) +
  ...
  geom_sf(data = ctrl_sites) +
  geom_sf(data = exp_sites) +
  geom_sf(data = st_sfc(st_cast(do.call(c, st_geometry(stations[c(path),])), 'LINESTRING'), crs = 4326))
...
ctrl_sites <- ctrl_area |> 
  ...
  st_as_sf() |> 
  st_transform(4326) |>
  DT(, station := paste('C0722', 1:2, sep = '_'))
...
ggplot() +
  geom_sf(data = hab, aes(fill = ECOLOGICAL)) +
  geom_sf(data = zones) +
  geom_sf(data = ctrl_area) +
  geom_sf(data = ctrl_sites) +
  geom_sf(data = exp_sites)
...
sta_hab <- st_buffer(stations[-1,], dist = units::set_units(5, mile)) |> 
...
ggplot() +
  ...
  geom_sf(data = stations[-1, ], size = 6, color = 'red') +
  geom_sf_label(data = stations[-1, ], aes(label = station), nudge_x = 2000, nudge_y = 2000) +
