hobo_clean <- function(raw_data_hobo, raw_data_pot){
  ...
  raw_hobo_xlsx <-  sapply(
    grep('\\.xlsx$', raw_data_hobo, value = T),
    read_xlsx,
    sheet = 'Data',
    simplify = F
  )
raw_hobo_xlsx
  ...
  # 
  # raw_hobo_csv <- sapply(
  #   grep('\\.csv$', raw_data_hobo, value = T),
  #   read.csv,
  #   simplify = F
  # )
  # raw_hobo_csv <- lapply(
  ...
  # )
  # 
  # raw_hobo <- bind_rows(c(raw_hobo_xlsx, raw_hobo_csv), .id = 'file_path')
  
  # hobo_clean <- raw_hobo |>
  #   dplyr::mutate(
  #     hobo_id = as.numeric(gsub('.*/| 202.*', '', file_path)),
  #     cruise = gsub('^.*\\d{8} |-\\d{2} .*$', '', file_path)
  #   )
  # 
  ...
  # 
  # hobo_clean |>
  #   select(cruise, station, hobo_id, datetime_edt, bwt_c, depth = deploy_depth, file_path)
}
