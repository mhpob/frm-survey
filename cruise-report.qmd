...
library(dplyr)
library(targets)
exp_tracks <- lapply(list.files('data/gps 2206', full.names = T),
...
The individual tracks can be seen by zooming in. Note the variability in the real track (see experimental drops) versus point-to-point tracks (see the control sites, where real tracks weren't recorded. Additionally, we can see that we went straight over the control reef, but drifted around a bit at the experimental site.
